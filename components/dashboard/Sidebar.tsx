...
export function Sidebar({ user }: { user?: any }) {
  ...
  return (
    <>
      ...
      {/* ── Sidebar ── */}
      <aside className={`fixed top-0 h-screen w-64 bg-white ${isRTL ? "right-0 border-l" : "left-0 border-r"} border-gray-100 flex flex-col z-50 overflow-hidden transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : isRTL ? "max-lg:translate-x-full" : "max-lg:-translate-x-full"} lg:transform-none lg:translate-x-0`}>
        ...
        {/* ── Bottom: Language switcher + user card + sign out ── */}
        <div className="px-3 pb-4 shrink-0 border-t border-gray-100 pt-3 space-y-2">
          ...
          >
            ...
            <span>{t.dashboardNav?.signOut || "تسجيل الخروج"}</span>
          </button>
        </div>
      </aside>
    </>
