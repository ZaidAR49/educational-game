...
export function GameplayScreen({
  ...
  const scoreLabel = t.game.play.scoreLabel || gamePlay?.scoreLabel
  const questionLabel = t.game.play.questionLabel || gamePlay?.questionLabel
  const skipButtonLabel = t.game.play.skipButtonLabel || gamePlay?.skipButtonLabel
  const nextButtonLabel = t.game.play.nextButtonLabel || gamePlay?.nextButtonLabel
  const showResultsLabel = t.game.play.showResultsLabel || gamePlay?.showResultsLabel
  ...
  return (
    <div className="bg-white rounded-3xl p-4 sm:p-6 shadow-xl relative animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-4 gap-2">
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-blue-50 px-4 py-2 rounded-xl text-center min-w-[80px]">
          <span className="block text-xs text-gray-500 font-bold">{questionLabel}</span>
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all duration-300 text-start font-bold focus:outline-none focus:ring-4 focus:ring-indigo-300
              ...
              ${
                ...
                (choice.isCorrect ?? (choice.points > 0))
              ...
              ${
                ...
                selectedChoiceIndex === index &&
                !(choice.isCorrect ?? (choice.points > 0))
                  ? "border-amber-500 bg-amber-50 animate-shake"
                  : ""
              }
              ${
                hasAnswered &&
