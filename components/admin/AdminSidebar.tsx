...
export function AdminSidebar({ user }: AdminSidebarProps) {
  ...
  const navItems = [
    ...
    { name: t.adminNav.notifications, href: "/admin/notifications", icon: Bell },
  ...
  return (
    <>
      ...
      <button 
        ...
        className={`md:hidden fixed top-5 ${isRTL ? "right-4" : "left-4"} z-30 p-2 bg-slate-800 text-white rounded-lg shadow-md hover:bg-slate-700 transition-colors`}
      ...
      <aside className={`w-64 bg-slate-900 text-white flex-col flex fixed h-full ${isRTL ? "right-0" : "left-0"} z-50 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : isRTL ? "max-md:translate-x-full" : "max-md:-translate-x-full"} md:transform-none md:translate-x-0`}>
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">{t.adminNav.backToHome}</p>
            </div>
          </Link>
          <button 
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          >
            ...
            <span>{t.adminNav.teacherDashboard}</span>
          </Link>
        </nav>
