    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      ...
      >
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h3 className="text-lg font-bold text-slate-800">{t.adminSettings.modalEditTitle}</h3>
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-slate-400 mt-1">{t.adminSettings.emailImmutableHint}</p>
            </div>
            <div>
              ...
              {admin.role === "super_admin" && superAdminCount <= 1 && (
                <p className="text-xs text-amber-600 mt-1">{t.adminSettings.onlySuperAdminWarning}</p>
              )}
            </div>
          </div>
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            ...
            <button type="submit" disabled={isPending} className="px-6 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-medium transition-colors shadow-sm disabled:opacity-70 flex items-center gap-2">
              {isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : t.adminSettings.saveChanges}
            </button>
          </div>
        </form>
      </motion.div>
