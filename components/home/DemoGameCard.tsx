...
export function DemoGameCard() {
  ...
  const demoQuestion = {
    ...
    description: d.questionDescription,
    choices: [
      {
        id: 1,
        text: d.choice1Text,
        isCorrect: false,
        feedback: { title: d.choice1FeedbackTitle, message: d.choice1FeedbackMessage, tip: d.choice1FeedbackTip },
      },
      {
        id: 2,
        text: d.choice2Text,
        isCorrect: true,
        feedback: { title: d.choice2FeedbackTitle, message: d.choice2FeedbackMessage, tip: d.choice2FeedbackTip },
      },
      {
        id: 3,
        text: d.choice3Text,
        isCorrect: false,
        feedback: { title: d.choice3FeedbackTitle, message: d.choice3FeedbackMessage, tip: d.choice3FeedbackTip },
      },
      {
        id: 4,
        text: d.choice4Text,
        isCorrect: false,
        feedback: { title: d.choice4FeedbackTitle, message: d.choice4FeedbackMessage, tip: d.choice4FeedbackTip },
      },
    ],
  }
      ...
      <div className="relative bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-gray-100 p-6 md:p-8 overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-6 border-b border-gray-50 pb-4">
          <div className="flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-xl font-bold text-sm">
            ...
            <span>{score} {d.scoreLabel}</span>
          </div>
          <div className="flex items-center gap-1.5 text-gray-400 font-bold text-xs">
            <span>{d.questionCounter}</span>
            <Gamepad2 className="w-4 h-4 text-blue-500" />
          </div>
        </div>

        {/* Question — text-start: right in RTL, left in LTR */}
          ...
          {demoQuestion.choices.map((choice) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 font-bold transition-all duration-300 text-start ${style}`}
              >
        ...
        <AnimatePresence>
          {selectedChoice && (
            <motion.div
              ...
              className="mt-6 pt-5 border-t border-gray-100 text-start"
            >
              <div
